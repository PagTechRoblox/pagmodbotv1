    ...
    async run(client, message, args) {
        ...
        if (!target) {
            return message.channel.send(new Discord.MessageEmbed()
                .setColor('RED')
                .setDescription(`** *:no_entry_entry: Incorrect Usage, the correct usages are:\n\`${this.options.usage}\`* **`)
                .setTimestamp()
        ...
        if (member) {
            if (message.author.id !== message.guild.ownerID && message.member.roles.highest.comparePositionTo(member.roles.highest) <= 0) {
                return message.channel.send(new Discord.MessageEmbed()
                    ...
                    .setTimestamp()
                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true }) || client.user.displayAvatarURL({ dynamic: true }))
                    .setDescription('** *:no_entry_sign: entry_sign: I cant unmute this user. Contact an Server Administrator!* **')
                );
            }
        } else {
            return message.channel.send(new Discord.MessageEmbed()
                .setColor('RED')
                .setDescription(`** *:no_entry_sign: You cant unmute yourself!* **`)
                .setTimestamp()
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true }) || client.user.displayAvatarURL({ dynamic: true }))
            );
        }
        ...
        try {
            ...
            message.channel.send(`** *:white_check_mark: ${target} Was unmuted!* **`);
        } catch (e) {
            message.channel.send(`** *:no_entry_sign: Error occured on unmuting ${target}* **`);
        }
    }
}