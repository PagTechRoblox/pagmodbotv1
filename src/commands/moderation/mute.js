...
module.exports = class extends BaseCommand {
    ...
    async run(client, message, args) {
        ...
        if (target.equals(message.author)) {
            return message.channel.send(new Discord.MessageEmbed()
                .setColor('RED')
                .setDescription(`** *:no_entry_sign: You cant mute yourself!* **`)
                .setTimestamp()
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true }) || client.user.displayAvatarURL({ dynamic: true }))
            );
        }
        ...
        if (member) {
            if (message.author.id !== message.guild.ownerID && message.member.roles.highest.comparePositionTo(member.roles.highest) <= 0) {
                return message.channel.send(new Discord.MessageEmbed()
                    ...
                    .setTimestamp()
                    .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true }) || client.user.displayAvatarURL({ dynamic: true }))
                    .setDescription('This user Is admin/mod I cant do that!')
                );
            }
        } else {
            return message.channel.send(new Discord.MessageEmbed()
                .setColor('RED')
                .setDescription(`** *This user dosent exist!* **`)
                .setTimestamp()
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true }) || client.user.displayAvatarURL({ dynamic: true }))
            );
        }
        ...
        message.channel.send(`** *:white_check_mark: ${target} was muted* ** | *${_reason}*`);
        const modchannel = await client.channels.fetch(message.guild.db.moderation('mod-logs')).catch(() => { });
        if (modchannel && modchannel.type === 'mod-logs') {
