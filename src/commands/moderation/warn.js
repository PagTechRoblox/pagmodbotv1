...
module.exports = class extends BaseCommand {
    ...
    async run(client, message, args) {
        ...
        if (target.equals(message.author)) {
            return message.channel.send(new Discord.MessageEmbed()
                .setColor('RED')
                .setDescription(`** *:no_entry_sign: You can't warn yourself!* **`)
                .setTimestamp()
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true }) || client.user.displayAvatarURL({ dynamic: true }))
            );
        }
        ...
        if (member) {
            ...
            return message.channel.send(new Discord.MessageEmbed()
                .setColor('RED')
                .setDescription(`** *:no_entry_sign: This user doesn't exist* **`)
                .setTimestamp()
                .setFooter(message.author.tag, message.author.displayAvatarURL({ dynamic: true }) || client.user.displayAvatarURL({ dynamic: true }))
            );
        }
        ...
        try {
            ...
            message.channel.send(`** *:white_check_mark: ${target} Has been warned* ** | *${_reason}*`);
        } catch (e) {
            return message.channel.send('Member was not warned. unexpected error occured.');
        }
        const modChannel = await client.channels.fetch(message.guild.db.moderation('modLogs')).catch(() => { });
        if (modChannel) {
